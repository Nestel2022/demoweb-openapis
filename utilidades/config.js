export const ENV = "PROD";
export const isIDE = false;

const commonConfig = {
  logsEnabled: false
};

const config = {
  UAT: {
    ...commonConfig,
    userInfo: "https://superapp-api-test.miniprogramas.claro.com.co/"
  },
  PROD: {
    ...commonConfig,
    userInfo: "https://superapp-api.miniprogramas.claro.com.co/"
  },
  PREPROD: {
    ...commonConfig,
    userInfo: "https://superapp-api.miniprogramas.claro.com.co/"
  }
};

const GWPConfig = {
  UAT: {
    clientId: "2025070414560001",
    merchantId: "143",
    privateKey: ""
  },
  PROD: {
    clientId: "2025082009570001",
    merchantId: "137",
    privateKey: ""
  },
  PREPROD: {
    clientId: "2025081217290001",
    merchantId: "135",
    privateKey: ""
  }
};

export function getConfig(env) {
  if (config[env]) {
    return config[env];
  }

  return config.PROD;
}

export const { logsEnabled, userInfo } = getConfig(ENV);

export function getPrivateKey() {
  return GWPConfig[ENV] || GWPConfig.PROD;
}

export function getOpenApiUrls() {
  const base = userInfo.endsWith("/") ? userInfo : `${userInfo}/`;

  return {
    urlApplyToken: `${base}miniprogram/api/v2/authorization/`,
    urlUsers: `${base}miniprogram/api/v2/users/`
  };
}
